...
const StudentSignUpForm = (props) => {
  ...
  const timeDisplay = ( db_time ) => { // this converts the time to dd,hh,mm,ss
  ...
  const _getInputs = (event) => {
    ...
    const auction_convert = (days) => {
      let time = moment.utc().add(days, 'days').format()
      time = time.split('Z')
    ...
    axios.post(links.root + "students/create", fd).then(res => {
    ...
    return (
      <div className="photo-container">
        ...
        <div className="show-data">
          <ul>
            <li>Reserve Price: ${reserve}</li>
            <li >Time Till Employment: <span className="time-left">{day} days, {hour} hours, {minute} minutes, {second} seconds</span></li>
            <li>Bids: 0 </li>
            <li><Link
              to={{
                pathname: `/profile/${ id }`,
